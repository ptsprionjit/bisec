...
export const settingSlice = createSlice({
  ...
  reducers: {
    ...
    app_short_name: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_short_name.value = action.payload
      }
      updateTitle(state.setting.app_short_name.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_bn_name: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_bn_name.value = action.payload
      }
      updateTitle(state.setting.app_bn_name.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_short_bn_name: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_short_bn_name.value = action.payload
      }
      updateTitle(state.setting.app_short_bn_name.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_bn_address: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_bn_address.value = action.payload
      }
      updateTitle(state.setting.app_bn_address.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_en_address: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_en_address.value = action.payload
      }
      updateTitle(state.setting.app_en_address.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_phone: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_phone.value = action.payload
      }
      updateTitle(state.setting.app_phone.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
    app_email: (state, action) => {
      if (typeof action.payload !== typeof undefined) {
        state.setting.app_email.value = action.payload
      }
      updateTitle(state.setting.app_email.value)
      updateStorage(state.saveLocal, state.storeKey, createSettingObj(state))
    },
