...
const TcPendingList = () => {
   ...
   const [userData, setUserData] = useState({ st_class: '', st_year: '', list_type: '13' });
   ...
   const fetchApplicationList = async () => {
      ...
      try {
         ...
         if (response.status === 200) {
            ...
            setValidated(false);
            setValidList(false);
            setStatus({ loading: false, success: false, error: "কোন অপেক্ষমান আবেদন পাওয়া যায়নি!" });
         }
      } catch (err) {
         if (err.status === 401) {
   ...
   const searchReset = () => {
      setValidated(false);
      setUserData({ st_class: '', st_year: '', list_type: '13' });
      setUserDataError([]);
      setValidList(false);
               ...
               <Col md="12">
                  <Card>
                     <Card.Header className="d-flex justify-content-between">
                        <Row>
                           <Col md={12}>
                              <h4 className="card-title text-center text-primary flex-fill"><span className={styles.SiyamRupaliFont + " text-center"}>ছাড়পত্রের (TC) অপেক্ষমান আবেদনসমূহ</span></h4>
                           </Col>
                           <Col md={12}>
