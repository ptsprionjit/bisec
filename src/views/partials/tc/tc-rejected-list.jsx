...
const TcRejectedList = () => {
   ...
   const fetchTcList = async () => {
      ...
      try {
         ...
         if (response.status === 200) {
            ...
            setValidated(false);
            setStError("কোন বাতিলকৃত আবেদন পাওয়া যায়নি!");
         }
      } catch (err) {
         if (err.status === 401) {
            navigate("/auth/sign-out");
   ...
   //    try {
   //       await axios.post(`${BACKEND_URL}/student/tc/application/reject?`, { id_invoice });
   //       setStData(((prevData) => prevData.filter((item) => item.id_invoice !== id_invoice)));
                  ...
                  <Card>
                     <Card.Header className="d-flex justify-content-between">
                        <div className="header-title d-flex flex-column justify-content-center align-items-center w-100">
                           <h4 className="card-title text-center text-primary flex-fill"><span className={styles.SiyamRupaliFont + " text-center"}>বাতিলকৃত ট্রান্সফার সার্টিফিকেট (TC) আবেদনসমূহ</span></h4>
                        ...
                        {stData.length > 0 && <>
                           <Row className='d-flex flex-row justify-content-between align-items-center px-4'>
                              <Col md="2">
                                 ...
                                 >
                                    <option disabled value=""><span className={styles.SiyamRupaliFont}>-- আবেদন সংখ্যা সিলেক্ট করুন --</span></option>
