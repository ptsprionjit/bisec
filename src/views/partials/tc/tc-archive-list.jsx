...
const TcArchiveList = () => {
   ...
   const [userData, setUserData] = useState({ st_class: '', st_year: '' });
   ...
   const fetchApplicationList = async () => {
      ...
      try {
         const response = await axiosApi.post(`/student/tc/archive/list`, { userData: userData });
         if (response.status === 200) {
            ...
            setValidated(false);
            setValidList(false);
            setStatus({ loading: false, success: false, error: "কোন আর্কাইভ আবেদন পাওয়া যায়নি!" });
         }
      } catch (err) {
         if (err.status === 401) {
               ...
               <Col md="12">
                  <Card>
                     <Card.Header className="d-flex justify-content-between">
                        <Row>
                           <Col md={12}>
                              <h4 className="card-title text-center text-primary flex-fill"><span className={styles.SiyamRupaliFont + " text-center"}>ছাড়পত্রের (TC) আর্কাইভ আবেদনসমূহ</span></h4>
                           </Col>
                           <Col md={12}>
