...
const TcApplication = () => {
   ...
   const fetchInstitute = async () => {
      setStatus({ loading: 'প্রতিষ্ঠানের তথ্য লোড হচ্ছে...অপেক্ষা করুন...', success: false, error: false });
      try {
         const response = await axiosApi.post(`/institute/list`, {});
         ...
         setStatus({ loading: false, success: false, error: err?.response?.data?.message || "প্রতিষ্ঠানের তথ্য পাওয়া যায়নি! কিছুক্ষণ পর আবার চেষ্টা করুন!" });
      } finally {
   ...
   const handleInstDataChange = (instData) => {
      instData?.value ? setUserData((prev) => ({ ...prev, 'id_institute': instData.value, 'en_institute': String(instData.label).split('-[')[0].replaceAll("'", "") })) : setUserData((prev) => ({ ...prev, 'id_institute': '', 'en_institute': '' }));
   }
   ...
   const printApplication = (() => {
      window.open(`/student/tc/application/print?invoiceNo=${stData.id_invoice}&prev_location=/student/tc/new`, '_blank', 'noopener noreferrer');
   });

   //Print TC Order
   const tcOrderPrint = (() => {
      window.open(`/student/tc/order?invoiceNo=${stData.id_invoice}&prev_location=/student/tc/new`, '_blank', 'noopener noreferrer');
   });
   ...
   const fetchStudentData = async () => {
      setStatus({ loading: "শিক্ষার্থীর তথ্য লোড হচ্ছে... অপেক্ষা করুন...", success: false, error: false });
      try {
         const response = await axiosApi.post(`/student/tc/data`, { userData: userData });
         if (response.status === 200) {
            ...
            setStatus({ loading: false, success: false, error: "শিক্ষার্থীর তথ্য সঠিক নয়!" });
         }
      } catch (err) {
         setStatus({ loading: false, success: false, error: "শিক্ষার্থীর তথ্য সঠিক নয়!" });
      } finally {
   ...
   const searchSubmit = (event) => {
      ...
      if (form.checkValidity() === false) {
         setValidated(false);
         setStatus({ loading: false, success: false, error: 'শিক্ষার্থীর তথ্য সঠিকভাবে পূরণ করতে হবে!' });
      } else {
   ...
   const transferSubmit = (event) => {
      ...
      if (form.checkValidity() === false) {
         ...
         const requiredFields = userData.id_institute ? ['id_institute'] : ['dst_board', 'en_dist', 'id_uzps', 'id_institute', 'en_institute'];
   ...
   const transferReset = (event) => {
      ...
      setUserData((prev) => ({ ...prev, 'dst_board': '11', 'en_dist': '', 'id_uzps': '', 'id_institute': '', 'en_institute': '' }));
      setUserDataError([]);
   };

   // Handle Unpaid Application
   const hanldeReAppPay = async (data) => {
      if (data.id_payment === "03") {
         setPendingResume(false);
      } else {
         if (data.dst_board === '11') {
            setUserData((prev) => ({ ...prev, 'dst_board': data.dst_board, 'id_institute': data.dst_eiin, 'en_institute': String(data.dst_institute).split('-[')[0].replaceAll("'", "") }));
            setPendingApplication(`${data.dst_institute}(${data.dst_eiin}) প্রতিষ্ঠানের জন্য আনপেইড আবেদন পাওয়া গেছে। প্রতিষ্ঠান পরিবর্তন করতে চাইলে আবার প্রতিষ্ঠান সিলেক্ট করুন অথবা প্রতিষ্ঠান একই হলে সরাসরি সাবমিট বাটনে ক্লিক করুন।`);
         } else {
            setUserData((prev) => ({ ...prev, 'dst_board': data.dst_board, 'id_institute': data.dst_eiin, 'en_institute': String(data.btc_institute).split('-[')[0].replaceAll("'", "") }));
            setPendingApplication(`${data.btc_institute}(${data.dst_eiin}) প্রতিষ্ঠানের জন্য আনপেইড আবেদন পাওয়া গেছে। প্রতিষ্ঠান পরিবর্তন করতে চাইলে আবার প্রতিষ্ঠান এন্ট্রি করুন অথবা প্রতিষ্ঠান একই হলে সরাসরি সাবমিট বাটনে ক্লিক করুন।`);
         }
      ...
      if (form.checkValidity() === false) {
         ...
         if (!isValid) {
            ...
            const payInitiate = async () => {
               setStatus({ loading: "পেমেন্ট এর জন্য তথ্য প্রসেস হচ্ছে... অপেক্ষা করুন...", success: false, error: false });
               try {
                  const response = !pendingResume ? await axiosApi.post(`/student/tc/payment/initiate`, { userData: myData }) : await axiosApi.post(`/student/tc/payment/resume`, { userData: myData });
                  if (response.status === 200) {
                  ...
                  setValidated(false);
                  setStatus({ loading: false, success: false, error: err?.response?.data?.message || "পেমেন্ট গেটওয়েতে সমস্যা। কিছুক্ষণ পর আবার চেষ্টা করুন!" });
               } finally {
                  setStatus((prev) => ({ ...prev, loading: false }));
               }
            }
   ...
   const resetPayment = (event) => {
      ...
      setUserData((prev) => ({ ...prev, 'st_mobile': '', 'st_email': '' }));
      setUserDataError([]);
   }
   ...
   useEffect(() => {
      setUserData((prev) => ({ ...prev, 'en_dist': '', 'id_uzps': '', 'id_institute': '', 'en_institute': '' }));
      if (instituteList.length && userData.dst_board) {
   ...
   // Option Upazila List
   useEffect(() => {
      setUserData((prev) => ({ ...prev, 'id_uzps': '', 'id_institute': '', 'en_institute': '' }));
   ...
   useEffect(() => {
      setUserData((prev) => ({ ...prev, 'id_institute': '', 'en_institute': '' }));
      if (instituteList.length) {
         const filteredData = instituteList.filter(
            item => item.id_uzps === userData.id_uzps && item.id_institute !== stData.src_eiin
         );
         const newOptions = filteredData.map(data => ({
            ...
            <Col md="12">
               <Card className="card-transparent shadow-none d-flex justify-content-center mb-0 auth-card">
                  <Card.Header className="d-flex flex-column justify-content-center mb-1">
                     {!permissionData?.id && <Link to="/student/tc/new" className="navbar-brand d-flex justify-content-center align-items-start w-100 gap-3">
                        <Logo color={true} />
                        <h2 className="logo-title text-primary text-wrap text-center">{appName}</h2>
                     </Link>}
                     {permissionData?.id && <Link to="/dashboard/home" className="navbar-brand d-flex justify-content-center align-items-start w-100 gap-3">
                        <Logo color={true} />
                        <h2 className="logo-title text-primary text-wrap text-center">{appName}</h2>
                     </Link>}
                        ...
                        <Table className='table table-bordered'>
                           ...
                           <tbody>
                              ...
                              <tr>
                                 <td className='text-wrap p-1 m-0'><span className={styles.SiyamRupaliFont}>প্রতিষ্ঠানের নাম</span></td>
                                 <td className='text-wrap p-1 m-0'>:</td>
                                 <td colSpan={4} className='text-uppercase text-wrap p-1 m-0'>{btcData ? `${userData.en_institute}` : `${userData.en_institute} (${userData.id_institute})`}</td>
                              </tr>
                              <tr>
                        ...
                        <Form noValidate validated={validated} onSubmit={submitPayment} onReset={resetPayment}>
                           <Row className='py-2'>
                              <Col md={12} className='my-2'>
                                 <h6 className={"text-center py-2"}>সকল তথ্য সঠিক থাকলে মোবাইল নম্বর ও ইমেইল এন্ট্রি করে নিচের <strong className='text-success'>পেমেন্ট</strong> বাটনে ক্লিক করুন।</h6>
                              </Col>
                              ...
                              <Col md={12} className='my-2'>
                                 {status.loading && <h6 className="text-center text-primary">{status.loading}</h6>}
                              ...
                              <Col md={4}>
                                 <Button className='flex-fill w-100' type="reset" variant="btn btn-danger">রিসেট</Button>
                              </Col>
                              <Col md={4}>
                                 <Button onClick={() => setTcRequest(false)} className='flex-fill w-100' type="button" variant="btn btn-outline-warning">ফিরে যান</Button>
                              </Col>
                              <Col md={4}>
                                 <Button className='flex-fill w-100' type="submit" variant="btn btn-success">পেমেন্ট</Button>
                              </Col>
                           </Row>
                        </Form>
                     </Card.Body>
