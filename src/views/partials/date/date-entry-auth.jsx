...
const DateEntryFinal = () => {
   ...
   useEffect(() => {
      let mounted = true;
      (async () => {
         if (!(((permissionData?.office === '03' || permissionData?.office === '04' || permissionData?.office === '05') && (permissionData?.role === '14' || permissionData?.role === '15')) || permissionData?.role === '16' || permissionData?.role === '17' || permissionData?.role === '18')) {
   ...
   const [userData, setUserData] = useState({
      eco_code: '', new_code: '', online_code: '', user_session: '', user_class: '', user_group: '', user_version: '', user_application: '', dt_start: '', dt_end: '', pay_start: '', pay_end: '', dob_start: '', dob_end: '', print_start: '', print_end: '', id_status: '13'
   });
   ...
   const handleSearchSubmit = async (event) => {
      ...
      if (form.checkValidity() === false) {
         ...
         if (!isValid) {
            ...
            try {
               const response = await axiosApi.post(`/entry/date/search`, { userData });
   ...
   const handleUpdateAuthorize = async (event) => {
      event.preventDefault();
      const form = event.currentTarget;
      let isValid = true;
      ...
      if (form.checkValidity() === false) {
         ...
         if (!isValid) {
            ...
            setLoadingData("তথ্য অনুমোদন করা হচ্ছে! অপেক্ষা করুন...");
            try {
               const myData = {
               ...
               const response = await axiosApi.post(`/entry/date/auth`, { userData: myData });
               if (response.status === 200) {
                  setLoadingSuccess(response.data.message);
                  alert(response.data.message);
                  setUserData({ ...userData, id_status: '17' });
                  setValidated(false);
               } else {
                  setLoadingError(`${response.data.message}`);
                  setValidated(true);
   ...
   const handleUpdateReject = async (event) => {
      event.preventDefault();
      const form = event.currentTarget;
      let isValid = true;
         ...
         if (!isValid) {
            ...
            try {
               ...
               if (response.status === 200) {
                  setLoadingSuccess(response.data.message);
                  alert(response.data.message);
                  setUserData({ ...userData, id_status: '18' });
                  setValidated(false);
               } else {
                  setLoadingError(`${response.data.message}`);
                  setValidated(true);
      ...
      <Fragment>
         <Row>
            <Col md={12}>
               <Card className="m-1">
                  <Card.Body>
                     <h4 className={styles.SiyamRupaliFont + " text-center text-uppercase w-100 card-title"}>আবেদনের তারিখ এন্ট্রি অনুমোদন</h4>
                        ...
                        <Row>
                           <Col md={12} className='mb-2'>
                              <Form.Label className="text-primary text-center w-100">
                                 ...
                                 <h6 className="text-uppercase text-center text-warning">(কোন তারিখ প্রযোজ্য না হলে সেখানে যে কোন একটি সঠিক তারিখ থাকতে হবে)</h6>
                              </Form.Label>
                           </Col>
                           <Col md={4}>
                           ...
                           <Col md={12}>
                              <p className='my-4 text-center text-primary'>এন্ট্রিকৃত তারিখ</p>
                           </Col>
