...
const InstEstablishmentOrder = () => {
    ...
    const fetchDataList = async () => {
        ...
        try {
            const st_list = await axiosApi.post(`/institute/establishment/order_list`);
    ...
    const appOrderGen = async (app_data) => {
        ...
        if (!isValid) {
            ...
            try {
                ...
                const response = await axiosApi.post(`/institute/establishment/app_order`, { userData: myData });
                if (response.status === 200) {
                                                ...
                                                filteredData.map((item, idx) => (
                                                    <tr key={idx}>
                                                        ...
                                                        <td className='text-left align-top text-wrap'>
                                                            ...
                                                            {item.email_ref && <Button type="button" variant="btn btn-outline-secondary" onClick={() => window.open(`${FRONTEND_URL}/order-emails?id_email=${item.email_ref}&prev_location=/establishment/generate-order`, '_blank', 'noopener,noreferrer')}>তদন্তের আদেশ প্রিন্ট</Button>}
                                                        </td>
                                                        <td className='text-center align-top text-wrap'>
                                                            ...
                                                            <Button type="button" variant="btn btn-outline-primary" onClick={() => window.open(`${FRONTEND_URL}/payment/response/success?prev_location=/establishment/generate-order&invoiceNo=${item.id_invoice}`, '_blank', 'noopener,noreferrer')}>প্রিন্ট ভাউচার</Button>
                                                        </td>
                                                        <td className='text-center align-top text-wrap'>
