...
import LeaveAppPrint from './print/app-print.jsx';

import { useAuthProvider } from "../../../context/AuthContext.jsx";
import axiosApi from "../../../lib/axiosApi.jsx";

const LeaveAppReport = () => {
    ...
    const fetchDataList = async () => {
        ...
        try {
            const response = await axiosApi.post(`/leave/application/list`, { app_status: '14' });

            if (response.status === 200 && response?.data?.leaveList?.length) {
                const datalength = response.data.leaveList.length;
                setDataList(response.data.leaveList);
                //Set Data for Pagination
                setRowsPerPage(10);
                setTotalPage(Math.ceil(datalength / 10));
                setCurrentPage(1);
                setCurrentData(response.data.leaveList.slice(0, 10));
    ...
    useEffect(() => {
        ...
        if (!((Number(permissionData.role) >= 14 && Number(permissionData.role) <= 18) || permissionData.type === '14' || permissionData.type === '15')) {
    ...
    const filteredData = useMemo(() => {
        ...
        return dataList.filter((item) =>
            [item.user_leave, item.user_duty, item.mobile.toString()].some((field) =>
    ...
    const applicationAuthorize = async (userData) => {
        ...
        const requiredFields = [
            'id_leave', 'total_requested', 'comment'
        ];
        ...
        if (!isValid) {
            ...
            try {
                const response = await axiosApi.post(`/leave/application/authorize`, { userData: userData });
                if (response.status === 200) {
                    setDataList(((prevData) => prevData.filter((item) => item.id_leave !== userData.id_leave)));
    ...
    const applicationReject = async (userData) => {
        ...
        if (!isValid) {
            ...
            try {
                const response = await axiosApi.post(`/leave/application/reject`, { userData: userData });
                if (response.status === 200) {
                    setDataList(((prevData) => prevData.filter((item) => item.id_leave !== userData.id_leave)));
                    setStatus({ loading: false, success: response.data.message, error: false });
                    setRejShow(false);
                ...
                <Col md={12} className='m-0 p-0'>
                    <Card className='m-0 p-3'>
                        <Card.Header className="d-flex flex-column justify-content-center align-items-center m-0 p-0">
                            <h4 className={"card-title text-center text-primary pb-2"}>ছুটির হিসাবায়নের অপেক্ষমান আবেদনসমূহ</h4>
                                    ...
                                    <thead>
                                        <tr className='bg-transparent'>
                                            ...
                                            <th className='text-center align-top text-wrap p-1 m-0'>
                                                <p className={"text-nowrap text-center"}>নাম</p>
                                                <p className={"text-nowrap text-center"}>পদবী</p>
                                            </th>
                                            <th className='text-center align-top text-wrap p-1 m-0'>
                                                <p className={"text-wrap text-center"}>দপ্তর</p>
                                                <p className={"text-wrap text-center"}>শাখা</p>
                                            </th>
                                            <th className='text-center align-top text-wrap p-1 m-0'>
                                                <p className={"text-wrap text-center"}>ছুটির ধরণ</p>
                                                <p className={"text-wrap text-center"}>ছুটির সময়</p>
                                            </th>
                                            <th className='text-center align-top text-wrap p-1 m-0'>
                                                <p className={"text-wrap text-center"}>আবেদনের অবস্থা</p>
                                            </th>
                                            <th className={"text-center align-top text-wrap p-0 m-0"}>সম্পাদনা</th>
                                        </tr>
                                    </thead>
                                    {dataList.length > 0 && <>
                                        <tbody>
                                                ...
                                                filteredData.map((item, idx) => (
                                                    <tr key={idx}>
                                                        ...
                                                        <td className='text-center align-top text-wrap'>
                                                            <p className={"text-left align-center text-wrap p-1 m-0"}>{item.bn_user_leave}</p>
                                                            <p className={"text-uppercase text-left align-center text-wrap p-1 m-0"}>{item.bn_post_leave}</p>
                                                        </td>
                                                        <td className='text-center align-top text-wrap'>
                                                            <p className={"text-center align-center text-wrap p-1 m-0"}>{item.bn_office_leave}</p>
                                                            <p className={"text-lowercase text-center align-center text-wrap p-1 m-0"}>{item.bn_section_leave}</p>
                                                        </td>
                                                        <td className='text-center align-top text-wrap'>
                                                            <p className={"text-center align-center text-wrap p-1 m-0"}>{item.bn_type}</p>
                                                            <p className={"text-center align-center text-wrap p-1 m-0"}>{InputValidation.E2BDigit(InputValidation.formatedDate(item.start_date))} থেকে {InputValidation.E2BDigit(InputValidation.formatedDate(item.end_date))}</p>
                                                        </td>
                                                        <td className='text-center align-top text-wrap'>
                                                            <p className={"text-center align-center text-wrap p-1 m-0"}>{item.bn_app_status}</p>
                                                        </td>
                                                        <td>
                                                            <div className="d-flex justify-content-center align-items-center flex-wrap gap-3">
                                                                <Button className='m-0 p-1' type="button" onClick={() => handleAuthorizeClick(item)} variant="btn btn-success" data-toggle="tooltip" data-placement="top" title="অনুমোদন" data-original-title="অনুমোদন">
                                                                ...
                                                                <Button className='m-0 p-1' type="button" onClick={() => handleRejectClick(item)} variant="btn btn-danger" data-toggle="tooltip" data-placement="top" title="বাতিল" data-original-title="বাতিল">
                                                                    <span className="btn-inner">
            ...
            {activeAppDetails && (
                ...
                >
                    ...
                    <Modal.Body className='m-0 p-0'>
                        <LeaveAppPrint
                            navigateLeaveAppPrint={detailsShow}
                            setNavigateLeaveAppPrint={setDetailsShow}
                            leavePrintData={activeAppDetails}
                            axiosApi={axiosApi}
                        />
                    </Modal.Body>
                    ...
                    <Modal.Body>
                        <Row>
                            <Form noValidate>
                                ...
                                {activeAppAuthorize.id_status === '14' && <Col className='my-2' md={12}>
                                    <Form.Label htmlFor="total_requested" className='fw-bold'>মোট আইনসম্মত ছুটিঃ</Form.Label>
                                    <Form.Control
                                        type='text'
                                        className='bg-transparent text-uppercase w-100'
                                        id="total_requested"
                                        value={activeAppAuthorize.total_requested}
                                        isInvalid={validated && !!activeAppAuthorize.total_requested}
                                        isValid={validated && activeAppAuthorize.total_requested && !activeAppAuthorize.total_requested}
                                        onChange={
                                            (e) => {
                                                setActiveAppAuthorize({ ...activeAppAuthorize, total_requested: e.target.value });
                                            }
                                        }
                                    />
