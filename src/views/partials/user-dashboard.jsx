...
import { Row, Col, Dropdown, Button, Card, Modal } from "react-bootstrap";
...
const UserDashBoard = React.memo(() => {
  ...
  */
  ...
  const getData = useCallback(async () => {
    try {
      const res = await axiosApi.post(permissionData.type === '13' ? `/dashboard/institute` : `/dashboard/board`, {});
      if (res?.status === 200) {
  ...
  const buildCharts = useCallback(() => {
    ...
    const dataValueExm = [st_classQQ06, st_classQQ07, id_examQQ03 || id_examQQ04, st_classQQ09, id_examQQ05, st_classQQ11, id_examQQ07];
    const dataTitleReg = ["ষষ্ট (০৬)", "সপ্তম (০৭)", "অষ্টম (০৮)", "নবম (০৯)", "দশম (১০)", "একাদশ (১১)", "দ্বাদশ (১২)"];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!dashBoardData?.regData || !dashBoardData?.exmData || !dashBoardData?.dateData || !dashBoardData?.noticeData) {
        getData();
      } else {
        setData();
      }
                ...
                <div className="card-body">
                  <div className="progress-widget">
                    <Circularprogressbar
                      ...
                      Linecap="rounded"
                      style={{ width: 60, height: 60 }}
                      value={(exmData.active_total) / (regData.active_total || 1) * 100}
