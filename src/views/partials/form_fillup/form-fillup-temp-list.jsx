...
setSearchError("আবেদন বাতিল সফল হয়নি!");
   ...
   const handlePayment = async () => {
      const payInitiate = async () => {
         ...
         setPaySuccess(false);
         try {
            const response = await axios.post(`${BACKEND_URL}/registration/payment-initiate?`, { user_form: searchData.user_form, user_eiin: searchData.user_eiin, user_session: searchData.user_session, user_class: searchData.user_class, user_group: searchData.user_group, user_version: searchData.user_version, user_application: searchData.user_application, user_paid: searchData.user_paid, user_st_total: String(stData.length), user_pay_total: String(totalAmount), prev_invoice: prevInvoice });
