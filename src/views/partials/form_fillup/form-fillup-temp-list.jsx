...
const FormFillupTempList = () => {
   ...
   const handlePayment = async () => {
      const payInitiate = async () => {
         ...
         setPaySuccess(false);
         try {
            const response = await axiosApi.post(`/registration/payment-initiate?`, { user_form: searchData.user_form, user_eiin: searchData.user_eiin, user_session: searchData.user_session, user_class: searchData.user_class, user_group: searchData.user_group, user_version: searchData.user_version, user_application: searchData.user_application, user_paid: searchData.user_paid, user_st_total: String(stData.length), user_pay_total: String(totalAmount), prev_invoice: prevInvoice });
   ...
   if (regPay && validSearch && profileImageList && (permissionData.type === '13' || permissionData.office === '05' || permissionData.role === '17')) return (
      <Fragment>
         <Row>
            <Col md={12}>
               <Card className='p-0'>
                  <Card.Body className='py-1 m-0 mb-2 d-flex flex-column justify-content-center align-items-center'>
                     ...
                     <Button type="button" onClick={() => setRegPay(false)} variant="btn btn-primary"><span className={styles.SiyamRupaliFont + " text-center"}>ফিরে যান</span></Button>
                  </Card.Body>
               </Card>
            </Col>
            <Col md="12">
            ...
            <Col md={12}>
               <Card className='p-0 mb-2'>
                  <Card.Body className='py-1 m-0 d-flex flex-column justify-content-center align-items-center'>
                     <h4 className={styles.SiyamRupaliFont + " text-center text-uppercase card-title pt-2"}>নিবন্ধিত (অপেক্ষমান/Pending) শিক্ষার্থীদের তালিকা</h4>
                     {insName && <h5 className="text-uppercase text-center py-1 text-primary">{insName}</h5>}
                     {searchLoading && <h6 className="text-uppercase text-center py-1 text-info">{searchLoading}</h6>}
                     {searchError && <h6 className="text-uppercase text-center py-1 text-danger">{searchError}</h6>}
                     {searchSuccess && <p className="text-uppercase text-center py-1 text-success"><small><i>{searchSuccess}</i></small></p>}
            ...
            <Col sm="12">
               <Card>
                  ...
                  <Card.Body className="px-0">
                     ...
                     {totalPage > 0 && <Row className='d-flex justify-content-center align-items-end mt-2 py-5 px-4 gap-4'>
                        <Col md={4} className='d-flex justify-content-center align-items-center'>
                           <Button onClick={() => handlePrint()} className='flex-fill' variant="btn btn-primary">
                              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-printer-check-icon lucide-printer-check"><path d="M13.5 22H7a1 1 0 0 1-1-1v-6a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v.5" /><path d="m16 19 2 2 4-4" /><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v2" /><path d="M6 9V3a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v6" /></svg> প্রিন্ট লিস্ট
                           </Button>
                        </Col>
                        {curDateTime >= datesData.pay_start && curDateTime <= datesData.pay_end && <Col md={4} className='d-flex justify-content-center align-items-center'>
...
export default FormFillupTempList;